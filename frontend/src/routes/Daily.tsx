      ...
      <div className="grid gap-4">
        <Card className="bg-zinc-900 border-zinc-800 backdrop-blur-sm overflow-hidden shadow-2xl">
          {tasksLoading ? (
            ...
            // I want this to show column headers for "Task", "Priority", "Urgency", and "Actions"
            <div>
              <CardHeader className="flex items-center p-1 border-b border-zinc-800">
                <span className="w-10 shrink-0" />
                <span className="flex-1 text-xs font-medium text-zinc-500 uppercase tracking-wider">Task</span>
                <span className="w-20 text-xs font-medium text-zinc-500 uppercase tracking-wider text-center">
                  Priority
                </span>
                <span className="w-20 text-xs font-medium text-zinc-500 uppercase tracking-wider text-center">
                  Urgency
                  ...
                  {tasks.map((task: Task) => (
                    ...
                    >
                      ...
                      <div className="flex-1 flex items-center justify-between min-w-0 transition-transform duration-300 ease-out group-hover:-translate-x-2 pr-4 pl-1 py-3 z-10">
                        ...
                        <div className="flex items-center gap-14 shrink-0 ml-4 group-hover:opacity-0 transition-opacity duration-200">
                          <PriorityBadge priority={task.user_priority} className="text-[10px] px-2 py-0.5" />
